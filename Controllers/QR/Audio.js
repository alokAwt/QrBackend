...
const CreateQr = async (req, res, next) => {
  try {
    ...
    let url = `https://qr-backend-ten.vercel.app/api/v1/Scan/Scanqr/Audio/${timestamp}${randomPart}`;
    ...
    qr.then(async (qr) => {
      req.body.QrImage = qr;
      //---  Save the Deatils
      let NewQr = await AudioModel.create(req.body);
...
// const AudioScanQr = async (req, res, next) => {
//   try {
//     //------------------Validation Error-------------------------//
//     let error = validationResult(req);
//     if (!error.isEmpty()) {
...
//     //-----------------Finding Qr-----------------------------//
//     let qr = await AudioModel.findOne({
...
const GetSingleQr = async (req, res, next) => {
  try {
    ...
    let Qr = await AudioModel.findOne({ _id: req.params.id });
    if (!Qr) {
...
const UpdateQrData = async (req, res, next) => {
  try {
    ...
    let updatedata = await AudioModel.findByIdAndUpdate(
      getQr,
      {
        Url: req.body.Url,
      },
...
const DeleteQr = async (req, res, next) => {
  try {
    ...
    //------------------------Finding User----------------------//
    let user = await UserModel.findById(req.user);
    let Qr = await AudioModel.findById(req.params.id);
